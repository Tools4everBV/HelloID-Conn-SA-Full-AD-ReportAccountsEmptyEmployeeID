...
<# Begin: DataSource "AD-user-generate-table-report-empty-employeeID" #>
$tmpPsScript = @'
try {
    $filter = {-not(employeeID -like "*")}
...
Invoke-HelloIDDelegatedForm -DelegatedFormName "AD - Report - Accounts where "employeeID" is empty" -DynamicFormGuid $dynamicFormGuid -AccessGroups $delegatedFormAccessGroupGuids -Categories $delegatedFormCategoryGuids -UseFaIcon "True" -FaIcon "fa fa-info-circle" -returnObject ([Ref]$delegatedFormRef) 
<# End: Delegated Form #>
	...
	$tmpScript = @'
            try {
                if($exportReport -eq "True") {
                    ...
                    $exportFile = $HIDreportFolder + "Report_AD_AccountsEmptyEmployeeID_" + $timeStamp + ".csv"
...
'@ 
	...
	Invoke-HelloIDAutomationTask -TaskName "AD-export-report-accounts-empty-employeeID" -UseTemplate "False" -AutomationContainer "8" -Variables $tmpVariables -PowershellScript $tmpScript -ObjectGuid $delegatedFormRef.guid -ForceCreateTask $true -returnObject ([Ref]$delegatedFormTaskGuid) 
} else {
	Write-ColorOutput Yellow "Delegated form 'AD - Report - Accounts where "employeeID" is empty' already exists. Nothing to do with the Delegated Form task..." 
}
